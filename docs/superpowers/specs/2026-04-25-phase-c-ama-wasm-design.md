...
実測で 5MB を超えたら `8 * 1024 * 1024` まで緩和。
...
- 実測 800ms 超過なら beam パラメータ調整(`tools/wasm_api.cpp` で `beam::Config{width: 150, depth: 12}` を渡す等。`beam::Config` の API シグネチャは実装時に確認)
...
| 1 手レイテンシ(M1 Chrome) | < 500ms | < 800ms |
| .wasm + .js サイズ(gzip 後) | < 3MB | < 5MB |
...
| 4 | 1 手レイテンシが 800ms を超える | UX 悪化 | `beam::Config` を `{width: 150, depth: 12}` に縮小して再測。さらに駄目なら `{width: 100, depth: 10}` |
| 5 | .wasm が 5MB を超え PWA precache に乗らない | 初回ロード失敗 | `maximumFileSizeToCacheInBytes` を 8MB まで緩める |
