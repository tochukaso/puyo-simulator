...
- **TF.js モデルサイズ**: 硬い制約なし。ただし参考値として現状 policy-ama-v1 が 4.8 MB、v2 試算が +600 KB(stem +9 KB / ResBlock 2 個追加 +590 KB)で約 5.4 MB。観察用の数値として残す。
- **AI 1 手提案レイテンシ ≤ 20 s**(K=6 expectimax の試算 800〜1400 ms に対して大幅な余裕。ユーザ操作の体感は気にせず、AI 思考時間として 20 秒まで許容)
- 上記レイテンシは「使い切る目標値」ではなく「天井」。実装後の実測が想定内なら追加最適化は不要。
...
- TF.js batched 推論で **800〜1400 ms / 1 手提案**(レイテンシ KPI 20 s に対して大幅な余裕)

K=5 / K=8 と比較した精度・性能は §3 の評価でクロスチェック(K を変えた eval を 1 回追加)。policy の top-K hit 率次第で「K=6 で頭打ち感」が出るはずなので、第一弾は K=6 固定で確定。性能余裕があるので、後で K=8 や chance 詳細化に振る選択肢は残しておく。
...
- ResNet 8 ブロック → **10 ブロック**(容量微増、追加 +590 KB で全体 ~5.4 MB)
...
K=6 / 475 NN forward / TF.js batched で 800〜1400 ms 試算。20 s KPI に対して大幅余裕。万一 20 s を超える場合のみ:
- batch サイズ調整
- K=5 にダウングレード
- chance node の代表を 1 個に集約(同色を捨てる)
- ResNet ブロック数を 8 のままに戻す
